...
void testApp::initSprites()
{
    ...
    float sideLen = 512/10.f;
    spriteRenderer = new ofxSpriteSheetRenderer(1, 10000, 0, sideLen);
    ...
    spriteRenderer->loadTexture("ltrs.png", 512, GL_NEAREST);
    
    int hCount = 50;
    int vCount = 20;
    float scalingX = ofGetScreenWidth()/hCount;
    float scalingY = ofGetScreenHeight()/vCount;
    ...
    if(sprites.size()>0) // if we have sprites
    {
        for(int i=sprites.size()-1;i>=0;i--) //go through them
        {
            spriteRenderer->addCenteredTile(&sprites[i]->animation,
                                            sprites[i]->pos.x,
                                            sprites[i]->pos.y); // add them to the sprite renderer (add their animation at their position, there are a lot more options for what could happen here, scale, tint, rotation, etc, but the animation, x and y are the only variables that are required)
        }
    }
...
//        newSprite->animation.index = (int)ofRandom(0,10)*10; //change the start index of our sprite. we have 4 rows of animations and our spritesheet is 8 tiles wide, so our possible start indicies are 0, 8, 16, and 24
//        sprites.push_back(newSprite); //add our sprite to the vector
//    }
}
